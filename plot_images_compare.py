...
def plot_compare(syn_batch, ref_batch, batch_path, number_of_plots=5, vmin=0, vmax=255, scale=True):
    ...
    random_images = random.sample(range(np.size(syn_batch, 0)), number_of_plots)
    syn_images = syn_batch[random_images]
    real_images = ref_batch[random_images]
    ...
    for plot_number in range(number_of_plots):
        figure_name = '{}.png'.format(plot_number)
